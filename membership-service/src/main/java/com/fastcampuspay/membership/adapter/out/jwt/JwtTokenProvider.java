...
public class JwtTokenProvider implements AuthMembershipPort {
	...
	private final String jwtSecret; // 내부 비밀키. 즉, jwt token 생성을 위한 비밀키에요.
	...
	public String generateJwtToken(Membership.MembershipId membershipId) {
		...
		return Jwts.builder()
			.setSubject(membershipId.getIdValue())
			.setIssuedAt(now)
	...
	public boolean validateJwtToken(String jwtToken) {
		try {
			Jwts.parserBuilder().setSigningKey(jwtSecret).build().parseClaimsJws(jwtToken).getBody();
			return true;
